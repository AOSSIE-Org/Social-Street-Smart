...
if(p['api'] === 'fn'){
  ...
  if(claims === undefined){
    ...
    ele.className = 'text-warning font-weight-bold';
    ele.innerText = 'Sorry, no related facts found';
    li.append(ele);
    resultList.append(li);
...
{
  console.log('shc goes here');
  
  // updating api name
  document.getElementById('apiName').innerText = 'Results for Security Header Checker API';

  // updating url 
  document.getElementById('urlText').innerHTML = '<b>URL :</b> ' +atob(p['target']);

  // updating results
  if(!(atob(p['valid'])))
  {
    document.getElementById('urlStat').innerText = 'Invalid URL';
    document.getElementById('urlStat').className = 'btn btn-sm btn-warning';
  }
  else if(atob(p['score']) < 1)
  {
    document.getElementById('urlStat').innerText = 'Unsafe';
    document.getElementById('urlStat').className = 'btn btn-sm btn-danger';
  }
  
}
else if (p['api'] === 'ssl') 
{
  console.log('ssl goes here');
 
  // updating api name
  document.getElementById('apiName').innerText = 'Results for SSL Validator API';
  ...
  else if(!(atob(p['safe'])))
  {
    ...
    document.getElementById('urlStat').innerText = 'Safe';
    document.getElementById('urlStat').className = 'btn btn-sm btn-success';
  }
