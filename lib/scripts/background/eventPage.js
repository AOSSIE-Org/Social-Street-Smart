...
chrome.contextMenus.onClicked.addListener(function(clickData){
  ...
  if(clickData.menuItemId === 'fnMenu'){
    ...
    if(clickData.selectionText){
      ...
      fetch('http://127.0.0.1:5000/getText',getText)
...
function saveToLocalAndDB(menuItemId,clickData){
  if( menuItemId === 'reportFNMenu'){
    ...
    if (clickData.selectionText){
      ...
      xhttp.open('POST', 'http://127.0.0.1:5000/reportfake', true);
    ...
    fetch('https://factchecktools.googleapis.com/v1alpha1/claims:search?query=' + query +'&key=' + creds, requestOptions)
      ...
      .then(result = (result) => {
        ...
        if(json_result['claims']){
          ...
          fetch('http://127.0.0.1:5000/savefc',saveResultOptions)
            .then((response) => {
...
function checkFakeNews(query){
  ...
  relatedArticles.then((data)=>{ 
    ...
    fetch('http://127.0.0.1:9999/predict', requestOptions)
      .then(response => response.text())
      .then(result  = (result) => {
        console.log('result' + result); 
        chrome.windows.create({
          url : 'scripts/content/dii/fnResult.html?api=fn&src=' + btoa(query) + '&result=' + btoa(result),
          focused : true,
          type : 'popup'});
      })
